...
static void anjay_task(void *pvParameters)
{
    ...
    if (anjay_attr_storage_install(anjay) || setup_security_object(anjay) || setup_server_object(anjay) || fw_update_install(anjay))
    {
        avs_log(tutorial, ERROR, "Failed to install core objects");
        return;
    }

    if (!(DEVICE_OBJ = device_object_create()) || anjay_register_object(anjay, DEVICE_OBJ))
    {
 ...
 */
#ifdef CELLULAR
    int ret = setupCellular();
    configASSERT(ret == 1); // restart the esp if this fails
#else
    ...
    xTaskCreate(&dummy_task, "dummy_task", 4096, NULL, 1, NULL);
}

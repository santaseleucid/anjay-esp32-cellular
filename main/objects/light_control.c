...
#define CONFIG_ANJAY_CLIENT_LIGHT_CONTROL 1 // SA - remove
/**
...
static int rgb_led_set(light_control_instance_t *inst)
{
    ...
    if (ledc_set_duty(LEDC_LOW_SPEED_MODE, inst->channel, real_power) || ledc_update_duty(LEDC_LOW_SPEED_MODE, inst->channel))
    {
...
static int init_instance(light_control_instance_t *inst,
                         ...
                         ledc_channel_t channel)
{
    printf("Light Control Create: init_instance\n");
...
{
    printf("Light Control Create: light_control_object_create\n");
    ...
    for (anjay_iid_t iid = 0; iid < LED_NUM; iid++)
    {
        ...
        if (ledc_channel_config(&channel_config) || rgb_led_set(&obj->instances[iid]))
        {

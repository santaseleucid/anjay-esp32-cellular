...
static handle_sockets_result_t handle_sockets(event_loop_state_t *state)
{
    ...
    assert(avs_time_duration_valid(state->max_wait_time) && !avs_time_duration_less(state->max_wait_time,
    ...
    if (anjay_sched_time_to_next(state->anjay_locked, &wait_time) || !avs_time_duration_less(wait_time, state->max_wait_time))
    {
        wait_time = state->max_wait_time;
    }
    assert(avs_time_duration_valid(wait_time) && !avs_time_duration_less(wait_time, AVS_TIME_DURATION_ZERO));
    ...
    if (result == HANDLE_SOCKETS_CONTINUE)
    {
        if (avs_time_duration_to_scalar(&wait_ms, AVS_TIME_MS, wait_time) || wait_ms > INT_MAX)
        {
    ...
    if (sockets_ready)
    {
        ...
        {
            if (state->allow_interrupt && !should_event_loop_still_run(state->anjay_locked))
            {
            ...
            if (fd == INVALID_SOCKET || !(FD_ISSET(fd, &infds) || FD_ISSET(fd, &errfds)))
            {
                continue;
            }
#endif // AVS_COMMONS_NET_POSIX_AVS_SOCKET_HAVE_POLL
...
{
    if (!avs_time_duration_valid(max_wait_time) || avs_time_duration_less(max_wait_time, AVS_TIME_DURATION_ZERO))
    {

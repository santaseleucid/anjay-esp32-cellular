                                    ...
                                    void **out_psk_buffer)
{
    ...
    if (((path = MAKE_RESOURCE_PATH(ANJAY_DM_OID_SECURITY, security_iid,
        ...
        avs_is_err((err = avs_stream_offset(membuf, &psk_offset))) || psk_offset < 0 || ((path = MAKE_RESOURCE_PATH(ANJAY_DM_OID_SECURITY, security_iid, secret_key_rid)), _anjay_dm_read_resource_into_stream(anjay, &path, membuf)))
    {
    ...
    size_t buffer_size;
    if (avs_is_ok(err) && avs_is_ok((err = avs_stream_membuf_take_ownership(
    ...
    const anjay_iid_t *security_iid)
{
    ...
    if (def->ensure_coap_context(server->anjay, connection) || avs_is_err((
...
static bool should_primary_connection_be_online(anjay_server_info_t *server)
{
    ...
    return !socket
           ...
           || server->registration_info.update_forced || _anjay_server_registration_expired(server)
    ...
    const anjay_server_name_indication_t *sni)
{
    ...
    if (server_info.transport_info && (!_anjay_socket_transport_supported(
    ...
    if (server_info.transport_info && primary_conn->transport != server_info.transport_info->transport)
    {
    ...
    avs_error_t err = refresh_connection(
        ...
        !!server_info.transport_info && should_primary_connection_be_online(server),
        &server_info);

                   ...
                   const anjay_connection_info_t *info)
{
    ...
    if (is_tls && dane_tlsa_record && avs_is_err((err = avs_net_socket_set_opt(socket, AVS_NET_SOCKET_OPT_DANE_TLSA_ARRAY, (avs_net_socket_opt_value_t){.dane_tlsa_array = {.array_ptr = dane_tlsa_record, .array_element_count = 1}}))))
    {
                                  ...
                                  anjay_server_connection_t *connection)
{
    ...
    if (!avs_coap_ctx_has_socket(connection->coap_ctx) && avs_is_err((err = avs_coap_ctx_set_socket(connection->coap_ctx,
                                                                                                    socket))))
    {
    ...
    else
    {
        ...
        printf("could not store bound local port");
    }
                                  ...
                                  anjay_server_connection_t *connection)
{
    if (anjay->udp_listen_port)
    {
        ...
        if (anjay->udp_listen_port && avs_simple_snprintf(static_preferred_port,
...
get_preferred_local_addr(const anjay_server_connection_t *connection)
{
    ...
    if (connection->nontransient_state.preferred_endpoint.size > 0 && avs_is_ok(avs_net_resolved_endpoint_get_host(
                                      ...
                                      anjay_server_connection_t *connection)
{
    ...
    avs_error_t err;
    if (avs_is_err(try_bind_to_last_local_port(connection, local_addr)) && avs_is_err((err = try_bind_to_static_preferred_port(
                                                                                           anjay, connection))))
    {
        return err;
    }

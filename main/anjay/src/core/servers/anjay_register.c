...
bool _anjay_server_primary_connection_valid(anjay_server_info_t *server)
{
    ...
    return _anjay_connection_get_online_socket((anjay_connection_ref_t){
               .server = server,
               .conn_type = ANJAY_CONNECTION_PRIMARY}) != NULL;
}
                           ...
                           void *args_)
{
    ...
    if (!obj_written && avs_is_err(avs_stream_write_f(args->stream, "</%u>", oid)))
    {
        return -1;
    }
    return 0;
                             ...
                             anjay_binding_mode_t *out_binding_mode)
{
    ...
    size_t out_ptr = 0;
    for (size_t in_ptr = 0; in_ptr < sizeof(*server_binding_mode) - 1 && server_binding_mode->data[in_ptr];
         ++in_ptr)
    {
        (void)lwm2m_version;
...
static anjay_registration_result_t map_coap_error(avs_error_t coap_err)
{
    ...
    if (coap_err.category == AVS_COAP_ERR_CATEGORY && coap_err.code == AVS_COAP_ERR_TIMEOUT)
    {
                          ...
                          anjay_update_parameters_t *move_params)
{
    ...
    avs_error_t err;
    if (avs_is_err((err = avs_coap_options_dynamic_init(&request.options))) || avs_is_err((err = setup_register_request_options(
                             ...
                             bool *out_dm_changed_since_last_update)
{
    ...
    const char *binding_mode = (strcmp(old_params->binding_mode.data,
                                       new_params->binding_mode.data) == 0)
                                   ? NULL
    ...
    *out_dm_changed_since_last_update =
    ...
    avs_error_t err;
    (void)((*out_dm_changed_since_last_update && avs_is_err((err = avs_coap_options_set_content_format(
                                                                 opts, AVS_COAP_FORMAT_LINK_FORMAT)))) ||
           avs_is_err(
               (err = _anjay_coap_add_string_options(
                        ...
                        anjay_update_parameters_t *move_params)
{
    ...
    avs_error_t err;
    if (avs_is_err((err = avs_coap_options_dynamic_init(&request.options))) || avs_is_err((err = setup_update_request_options(
                          ...
                          const anjay_update_parameters_t *new_params)
{
    ...
    return info->update_forced || old_params->lifetime_s != new_params->lifetime_s || strcmp(old_params->binding_mode.data, new_params->binding_mode.data) || !dm_caches_equal(old_params->dm, new_params->dm);
}
...
void _anjay_server_ensure_valid_registration(anjay_server_info_t *server)
{
    ...
    else
    {
        ...
        bool needs_update = !needs_reregistration && needs_registration_update(server, &new_params);
        if (needs_reregistration || (registration_or_update_in_progress && registration_expired && needs_update))
        {
            on_registration_update_result(server, &new_params,
                                          ANJAY_REGISTRATION_ERROR_REJECTED,
                                          avs_errno(AVS_UNKNOWN_ERROR));
                         ...
                         AVS_LIST(const anjay_string_t) endpoint_path)
{
    *out_request = (avs_coap_request_header_t){
    ...
    avs_error_t err;
    if (avs_is_err((err = avs_coap_options_dynamic_init(&out_request->options))) || avs_is_err((err = _anjay_coap_add_string_options(
                                                                                                    &out_request->options,
                                                                                                    endpoint_path,

    ...
    anjay_server_info_t *server)
{
    ...
    if (connection->state == ANJAY_SERVER_CONNECTION_STABLE && connection->stateful && !_anjay_disable_server_with_timeout_unlocked(server->anjay, server->ssid, AVS_TIME_DURATION_ZERO))
    {
        server->refresh_failed = true;
    }
    else
...
void _anjay_server_on_fatal_coap_error(anjay_connection_ref_t conn_ref,
                                       avs_error_t err)
{
    assert(avs_coap_error_recovery_action(err) == AVS_COAP_ERR_RECOVERY_RECREATE_CONTEXT);
    anjay_server_connection_t *conn =
        _anjay_connection_get(&conn_ref.server->connections,
                              conn_ref.conn_type);
    if (conn_ref.conn_type == ANJAY_CONNECTION_PRIMARY && conn->state != ANJAY_SERVER_CONNECTION_STABLE && _anjay_server_registration_expired(conn_ref.server))
    {
...
static bool should_retry_bootstrap(anjay_unlocked_t *anjay)
{
    ...
    return bootstrap_server_exists && (!possibly_active_server_exists || registration_failure_must_trigger_bootstrap);
}
...
int _anjay_servers_sched_reactivate_all_given_up(anjay_unlocked_t *anjay)
{
    ...
    AVS_LIST_FOREACH(it, anjay->servers)
    {
        if (_anjay_server_active(it) || !it->refresh_failed || (it->ssid == ANJAY_SSID_BOOTSTRAP && !_anjay_bootstrap_legacy_server_initiated_allowed(anjay)))
        {
            continue;
        }
                             ...
                             avs_time_duration_t reactivate_delay)
{
    ...
    if (ssid != ANJAY_SSID_BOOTSTRAP && !_anjay_bootstrap_in_progress(anjay) && _anjay_server_active(*server_ptr) && !_anjay_server_registration_expired(*server_ptr))
    {
    ...
    if (avs_time_duration_valid(reactivate_delay) && _anjay_server_sched_activate(*server_ptr, reactivate_delay))
    {
...
int _anjay_enable_server_unlocked(anjay_unlocked_t *anjay, anjay_ssid_t ssid)
{
    ...
    if (ssid == ANJAY_SSID_BOOTSTRAP && !_anjay_bootstrap_legacy_server_initiated_allowed(anjay) && _anjay_requested_bootstrap_action(anjay) == ANJAY_BOOTSTRAP_ACTION_NONE)
    {
        anjay_log(DEBUG,

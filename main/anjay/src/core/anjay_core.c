...
void _anjay_reschedule_coap_sched_job(anjay_unlocked_t *anjay)
{
    ...
    if (anjay->coap_sched)
    {
        ...
        {
            if (!anjay->coap_sched_job_handle || AVS_RESCHED_AT(&anjay->coap_sched_job_handle,
                                                                next_job_time))
            {
                         ...
                         anjay_request_t *out_request)
{
    ...
    if (parse_request_uri(hdr, &out_request->is_bs_uri, &out_request->uri) || parse_attributes(hdr, &out_request->attributes) || avs_coap_options_get_content_format(&hdr->options, &out_request->content_format) || parse_action(hdr, out_request))
    {
        return -1;
    }
    return 0;
...
static bool critical_option_validator(uint8_t msg_code, uint32_t optnum)
{
    ...
    case AVS_COAP_CODE_PUT:
    case AVS_COAP_CODE_POST:
        return optnum == AVS_COAP_OPTION_URI_PATH || optnum == AVS_COAP_OPTION_URI_QUERY;
    case AVS_COAP_CODE_DELETE:
        return optnum == AVS_COAP_OPTION_URI_PATH;
    default:
        return false;
                            ...
                            anjay_connection_ref_t connection)
{
    ...
    else if (recovery_action == AVS_COAP_ERR_RECOVERY_UNKNOWN && (err.category != AVS_COAP_ERR_CATEGORY || avs_coap_error_class(err) != AVS_COAP_ERR_CLASS_OTHER))
    {
...
{
    int time_to_next_ms;
    if (!anjay_sched_time_to_next_ms(anjay, &time_to_next_ms) && time_to_next_ms < limit_ms)
    {

...
static const char *user_state_get_version(anjay_unlocked_t *anjay,
                                          fw_user_state_t *user)
{
    if (!user->handlers->get_version || user->state != UPDATE_STATE_DOWNLOADED)
    {
        return NULL;
    }
    const char *result = NULL;
...
{
    assert(fw->user_state.state == UPDATE_STATE_IDLE || fw->user_state.state == UPDATE_STATE_DOWNLOADING);
...
#if defined(AVS_COMMONS_WITH_MBEDTLS) || defined(AVS_COMMONS_WITH_OPENSSL) || defined(AVS_COMMONS_WITH_TINYDTLS)
#define WITH_SSL
                              ...
                              void *fw_)
{
    ...
    else if (status.result != ANJAY_DOWNLOAD_FINISHED)
    {
        ...
        else if (status.result == ANJAY_DOWNLOAD_ERR_INVALID_RESPONSE && (status.details.status_code == AVS_COAP_CODE_NOT_FOUND || status.details.status_code == 404))
        {
        ...
        reset_user_state(anjay, fw);
        if (fw->retry_download_on_expired && status.result == ANJAY_DOWNLOAD_ERR_EXPIRED)
        {
            fw_log(INFO,
                             ...
                             const anjay_etag_t *etag)
{
    ...
    if (transport_security_from_uri(fw->package_uri) == ANJAY_TRANSPORT_ENCRYPTED)
    {
                    ...
                    anjay_unlocked_input_ctx_t *ctx)
{
    ...
    case FW_RES_PACKAGE_URI:
    {
        printf("1. FW_RES_PACKAGE_URI\n");
        assert(riid == ANJAY_ID_INVALID);
        printf("2. FW_RES_PACKAGE_URI\n");
        char *new_uri = NULL;
        ...
        if (!result && transport_security_from_uri(new_uri) == ANJAY_TRANSPORT_SECURITY_UNDEFINED)
        {
            fw_log(WARNING,
                        ...
                        void *fw_)
{
    ...
    if (!fw->update_job && fw->state == UPDATE_STATE_UPDATING && fw->user_state.state != UPDATE_STATE_UPDATING && AVS_SCHED_NOW(_anjay_get_scheduler_unlocked(anjay), &fw->update_job, perform_upgrade, &fw, sizeof(fw)))
    {
                   ...
                   const anjay_fw_update_initial_state_t *initial_state)
{
    ...
    case ANJAY_FW_UPDATE_INITIAL_DOWNLOADED:
        if (initial_state->persisted_uri && !(repr->package_uri =
    ...
    case ANJAY_FW_UPDATE_INITIAL_DOWNLOADING:
    {
                     ...
                     initial_state->resume_etag))
        {
            ...
            reset_user_state(anjay, repr);
            if (repr->result == ANJAY_FW_UPDATE_RESULT_CONNECTION_LOST && initial_state->resume_etag && schedule_background_anjay_download(anjay, repr, 0, NULL))
            {
                fw_log(WARNING, _("Could not retry firmware download"));
            }
        }
    ...
    const anjay_fw_update_initial_state_t *initial_state)
{
    ...
    ANJAY_MUTEX_LOCK(anjay, anjay_locked);
    AVS_LIST(fw_repr_t)
    repr = AVS_LIST_NEW_ELEMENT(fw_repr_t);
    if (!repr)
    {
    ...
    else
    {
        ...
        if (!initialize_fw_repr(anjay, repr, initial_state) && !_anjay_dm_module_install(anjay, &FIRMWARE_UPDATE_MODULE,
                                                                                         repr))
        {
...
{
    return result != ANJAY_FW_UPDATE_RESULT_INITIAL && result != ANJAY_FW_UPDATE_RESULT_SUCCESS;
}

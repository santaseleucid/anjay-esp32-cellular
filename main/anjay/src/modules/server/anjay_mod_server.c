                        ...
                        anjay_iid_t *inout_iid)
{
    ...
    if (instance->binding)
    {
        if (!anjay_binding_mode_valid(instance->binding) || avs_simple_snprintf(new_instance->binding.data,
                                     ...
                                     anjay_iid_t *inout_iid)
{
    ...
    else
    {
        ...
        if (!(retval = add_instance(repr, instance, inout_iid)) && (retval = _anjay_serv_object_validate(repr)))
        {
...
anjay_server_get_ssids(anjay_t *anjay_locked)
{
    assert(anjay_locked);
    AVS_LIST(server_instance_t)
    source = NULL;
...
int anjay_server_object_install(anjay_t *anjay_locked)
{
    ...
    int result = -1;
    ANJAY_MUTEX_LOCK(anjay, anjay_locked);
    AVS_LIST(server_repr_t)
    repr = AVS_LIST_NEW_ELEMENT(server_repr_t);
    if (!repr)
    {
        server_log(ERROR, _("out of memory"));
    }

                               ...
                               anjay_iid_t iid)
{
    ...
    AVS_LIST(sec_instance_t)
    created = AVS_LIST_NEW_ELEMENT(sec_instance_t);
    if (!created)
    {
        return ANJAY_ERR_INTERNAL;
    }
    ...
    anjay_iid_t *inout_iid)
{
    ...
    else
    {
        ...
        if (!(retval = add_instance(repr, instance, inout_iid)) && (retval = _anjay_sec_object_validate(anjay, repr)))
        {
...
int anjay_security_object_install(anjay_t *anjay_locked)
{
    ...
    ANJAY_MUTEX_LOCK(anjay, anjay_locked);
    AVS_LIST(sec_repr_t)
    repr = AVS_LIST_NEW_ELEMENT(sec_repr_t);
    if (!repr)
    {
        security_log(ERROR, _("out of memory"));
    }

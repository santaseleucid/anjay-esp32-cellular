...
typedef struct
{
    ...
    AVS_LIST(avs_coap_udp_unconfirmed_msg_t)
...
static size_t udp_max_payload_size(size_t buffer_capacity,
                                   ...
                                   size_t options_size)
{
    const size_t msg_size = (sizeof(avs_coap_udp_header_t) + token_size + options_size + sizeof(AVS_COAP_PAYLOAD_MARKER));
                                 ...
                                 const avs_coap_udp_msg_t *msg)
{
    ...
    bool has_block1 =
        (avs_coap_options_get_block(&msg->options, AVS_COAP_BLOCK1, &block1) == 0);
    ...
    bool has_block2 =
        (avs_coap_options_get_block(&msg->options, AVS_COAP_BLOCK2, &block2) == 0);
                               ...
                               const avs_coap_udp_msg_t *res)
{
    ...
    if ((type == AVS_COAP_UDP_TYPE_CONFIRMABLE || type == AVS_COAP_UDP_TYPE_NON_CONFIRMABLE) && _avs_coap_option_exists(&res->options,
                                ...
                                const avs_coap_udp_unconfirmed_msg_t *new_elem)
{
    ...
    AVS_LIST_ITERATE_PTR(list_ptr)
    {
        ...
        else if (new_elem->hold == (*list_ptr)->hold && avs_time_monotonic_before(new_elem->next_retransmit,
                                    ...
                                    avs_error_t fail_err)
{
    ...
    if (response && result == AVS_COAP_SEND_RESULT_OK && handler_result != AVS_COAP_RESPONSE_ACCEPTED)
    {
        AVS_LIST_INSERT(find_unconfirmed_insert_ptr(ctx, unconfirmed),
                        unconfirmed);
    }
                         ...
                         const uint16_t *id)
{
    ...
    AVS_LIST_ITERATE_PTR(list_ptr)
    {
        ...
        if ((direction == AVS_COAP_UDP_EXCHANGE_ANY || direction == direction_from_code(msg->header.code)) && (!token || avs_coap_token_equal(&msg->token, token)) && (!id || _avs_coap_udp_header_get_id(&msg->header) == *id))
        {
            return list_ptr;
        }
    }
...
retransmit_next_message_without_reschedule(avs_coap_udp_ctx_t *ctx)
{
    ...
    if (!unconfirmed || avs_time_monotonic_before(avs_time_monotonic_now(),
    ...
    assert(current_nstart(ctx) == AVS_MIN(AVS_LIST_SIZE(ctx->unconfirmed_messages),
                    ...
                    AVS_LIST(avs_coap_udp_unconfirmed_msg_t) unconfirmed)
{
    ...
    unconfirmed->hold =
        (AVS_LIST_NTH(ctx->unconfirmed_messages, ctx->tx_params.nstart - 1) != NULL);
...
{
    if (ctx->current_request.exists && avs_coap_code_is_response(msg->code) && avs_coap_token_equal(&msg->token, &ctx->current_request.token))
    {
        ctx->current_request.exists = false;
        if ((type == AVS_COAP_UDP_TYPE_ACKNOWLEDGEMENT || type == AVS_COAP_UDP_TYPE_RESET))
        {
                                   ...
                                   const avs_coap_udp_msg_t *msg)
{
    ...
    if (!unconfirmed_ptr)
    {
        bool is_confirmable = (_avs_coap_udp_header_get_type(&msg->header) == AVS_COAP_UDP_TYPE_CONFIRMABLE);
        LOG(DEBUG,
    ...
    if (err.category == AVS_COAP_ERR_CATEGORY)
    {
        ...
        case AVS_COAP_ERR_MALFORMED_OPTIONS:
        {
            ...
            else if (avs_coap_code_is_response(msg.header.code))
            {
                ...
                if (type == AVS_COAP_UDP_TYPE_CONFIRMABLE || type == AVS_COAP_UDP_TYPE_NON_CONFIRMABLE)
                {

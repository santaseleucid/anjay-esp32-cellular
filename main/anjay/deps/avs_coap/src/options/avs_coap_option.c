                                        ...
                                        const uint8_t *ext_value_ptr)
{
    ...
    {
    case _AVS_COAP_EXT_U8:
        return (uint32_t) * (const uint8_t *)ext_value_ptr + _AVS_COAP_EXT_U8_BASE;
    case _AVS_COAP_EXT_U16:
...
static inline bool ext_value_overflows(uint8_t base_value,
                                       const uint8_t *ext_value_ptr)
{
    return base_value == _AVS_COAP_EXT_U16 && extract_u16(ext_value_ptr) > UINT16_MAX - _AVS_COAP_EXT_U16_BASE;
}
...
{
    return opt->content + get_ext_field_size(_avs_coap_option_get_short_delta(opt));
}

const uint8_t *_avs_coap_option_value(const avs_coap_option_t *opt)
{
    return ext_length_ptr(opt) + get_ext_field_size(_avs_coap_option_get_short_length(opt));
}
                                  ...
                                  size_t max_opt_bytes)
{
    ...
    return required_bytes <= max_opt_bytes && !ext_value_overflows(short_delta, ext_delta_ptr(opt));
}
                                   ...
                                   size_t max_opt_bytes)
{
    ...
    uint8_t short_delta = _avs_coap_option_get_short_delta(opt);
    size_t required_bytes = 1 + get_ext_field_size(short_delta) + get_ext_field_size(short_length);
    return required_bytes <= max_opt_bytes && !ext_value_overflows(short_length, ext_length_ptr(opt));
}

bool _avs_coap_option_is_valid(const avs_coap_option_t *opt,
                               size_t max_opt_bytes)
{
    if (max_opt_bytes == 0 || !is_delta_valid(opt, max_opt_bytes) || !is_length_valid(opt, max_opt_bytes))
    {
        return false;
    }

    uint32_t length = (uint32_t)_avs_coap_option_sizeof(opt);
    return (uintptr_t)opt->content + length >= (uintptr_t)opt->content && length <= max_opt_bytes;
}

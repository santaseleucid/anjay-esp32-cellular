...
{
    assert(opt_number == AVS_COAP_OPTION_BLOCK1 || opt_number == AVS_COAP_OPTION_BLOCK2);
    ...
    if (_avs_coap_option_content_length(block_opt) > MAX_BLOCK_DATA_SIZE || _avs_coap_option_block_seq_number(block_opt, &out_info->seq_num) || _avs_coap_option_block_has_more(block_opt, &out_info->has_more) || _avs_coap_option_block_size(block_opt, &out_info->size, &out_info->is_bert))
    {
    ...
    bool *out_payload_marker_reached)
{
    ...
    *out_actual_size = (size_t)((const uint8_t *)it.curr_opt - (const uint8_t *)it.opts->begin);
    ...
    if (out_payload_marker_reached)
    {
        ...
        if (!all_bytes_parsed && *(const uint8_t *)it.curr_opt == AVS_COAP_PAYLOAD_MARKER)
        {
                                       ...
                                       uint16_t option_number)
{
    ...
    while (!_avs_coap_optit_end(&optit) && _avs_coap_optit_number(&optit) < option_number)
    {
                                 ...
                                 bool is_bert)
{
    ...
    uint32_t value = ((seq_number & 0x000fffff) << 4) | ((uint32_t)is_last_chunk << 3) | (uint32_t)size_exponent;
                                        ...
                                        uint16_t opt_data_size)
{
    ...
    while (!_avs_coap_optit_end(&insert_it) && _avs_coap_optit_number(&insert_it) <= opt_number)
    {
    ...
    size_t bytes_required =
        _avs_coap_get_opt_header_size(opt_num_delta, opt_data_size) + opt_data_size;
...
static bool is_option_identical(avs_coap_option_t *a, avs_coap_option_t *b)
{
    ...
    return len_a == len_b && memcmp(_avs_coap_option_value(a), _avs_coap_option_value(b),
                                    len_a) == 0;
}
                                      ...
                                      bool (*selector)(uint16_t))
{
    ...
    while (!_avs_coap_optit_end(&it_first) && !_avs_coap_optit_end(&it_second))
    {
...
static bool option_must_not_change_during_transfer(uint16_t opt_num)
{
    return (_avs_coap_option_is_critical(opt_num)
            ...
            // they are "critical"
            && opt_num != AVS_COAP_OPTION_BLOCK1 && opt_num != AVS_COAP_OPTION_BLOCK2)
    ...
    size_t expected_request_payload_offset)
{
    ...
    if (prev_response_has_block1 && !prev_response_has_block2)
    {
        ...
        return curr_request_has_block1 && block1_offset_matches(expected_request_payload_offset, curr);
    }

    /* Third and fourth case */
    if (prev_response_has_block2)
    {
        return !curr_request_has_block1 && curr_request_has_block2 && block2_offset_matches(prev_response, curr);
    }
...
validate_block2_in_block1_request(const avs_coap_options_t *opts)
{
    ...
    if (result == AVS_COAP_OPTION_MISSING || !_avs_coap_option_exists(opts, AVS_COAP_OPTION_BLOCK2))
    {
        return AVS_OK;
    }
...
{
    return option_must_not_change_during_transfer(opt_num) || opt_num == AVS_COAP_OPTION_BLOCK1 || opt_num == AVS_COAP_OPTION_BLOCK2;
}

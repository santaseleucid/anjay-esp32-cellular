    ...
    server_exchange_create(const server_exchange_create_args_t *args)
{
    ...
    const size_t request_key_options_capacity =
        _avs_coap_options_request_key_size(&args->request->options) + AVS_COAP_OPT_BLOCK_MAX_SIZE;
    // We may need to add both BLOCK1 and BLOCK2 to response options
    const size_t response_options_capacity =
        args->response_options->capacity + AVS_COAP_OPT_BLOCK_MAX_SIZE * 2 + (args->observe_option_value ? AVS_COAP_OPT_OBSERVE_MAX_SIZE : 0);

    AVS_LIST(avs_coap_exchange_t)
    exchange =
        (AVS_LIST(avs_coap_exchange_t))AVS_LIST_NEW_BUFFER(
            sizeof(avs_coap_exchange_t) + request_key_options_capacity + response_options_capacity);
    if (!exchange)
    {
        return NULL;
    }
    ...
    *exchange = (avs_coap_exchange_t){
        ...
        .eof_cache = {
            .empty = true},
        .by_type = {.server = {.request_handler = args->request_handler, .request_handler_arg = args->request_handler_arg, .reliability_hint = args->reliability_hint, .delivery_handler = args->delivery_handler, .delivery_handler_arg = args->delivery_handler_arg, .exchange_deadline = get_exchange_deadline(), .request_code = args->request->code, .request_key_options = _avs_coap_options_copy_request_key(&args->request->options, request_key_options_buffer, request_key_options_capacity), .request_key_options_buffer = request_key_options_buffer}},
        .options_buffer_size = response_options_capacity + request_key_options_capacity};
                                ...
                                AVS_LIST(avs_coap_exchange_t) * exchange_ptr)
{
    ...
    if ((*exchange_ptr)->by_type.server.reliability_hint == AVS_COAP_NOTIFY_PREFER_CONFIRMABLE)
    {
    ...
    if (avs_is_err(err))
    {
        if (err.category == AVS_COAP_ERR_CATEGORY && (err.code == AVS_COAP_ERR_MESSAGE_TOO_BIG || err.code == AVS_COAP_ERR_PAYLOAD_WRITER_FAILED))
        {
            err = send_ise(ctx, &token, handler, *exchange_ptr);
        }
        else if (err.category == AVS_COAP_ERR_CATEGORY && err.code == AVS_COAP_ERR_EXCHANGE_CANCELED)
        {
...
_avs_coap_async_server_abort_timedout_exchanges(avs_coap_ctx_t *ctx)
{
    ...
    while (coap_base->server_exchanges && avs_time_monotonic_before(coap_base->server_exchanges->by_type
                                     ...
                                     void *response_writer_arg)
{
    ...
    if (!ctx->observe_established && _avs_coap_option_exists(&response->options,
                                                             AVS_COAP_OPTION_OBSERVE))
    {
        LOG(ERROR,
...
{
    return request->payload_offset + request->payload_size == request->total_payload_size;
}
    ...
    avs_coap_ctx_t *ctx, avs_coap_exchange_t *exchange)
{
...
#ifdef WITH_AVS_COAP_OBSERVE
    if (handle_observe_option(ctx, &coap_base->request_ctx.request) == OBSERVE_REGISTER)
    {
        ...
        AVS_STATIC_ASSERT(sizeof(coap_base->request_ctx.request.token) == sizeof(*observe_id),
    ...
    const avs_coap_server_async_request_t async_request = {
        ...
        .payload_offset = get_request_payload_offset(&coap_base->request_ctx.request),
...
{
    return _avs_coap_option_is_critical(opt_number) || opt_number == AVS_COAP_OPTION_CONTENT_FORMAT;
}
                                     ...
                                     const avs_coap_exchange_t *exchange)
{
    ...
    if (request->payload_offset > 0 && avs_coap_token_equal(&request->token, &exchange->token) && _avs_coap_selected_options_equal(&request->options, &exchange->by_type.server.request_key_options, matching_request_options))
    {
        return true;
    }
#ifdef WITH_AVS_COAP_BLOCK
                                        ...
                                        const avs_coap_borrowed_msg_t *request)
{
    ...
    {
        if (avs_coap_code_is_response((*it)->code) && request_matches_exchange(request, *it))
        {
            return it;
        }
    }
...
static int validate_request_exchange_state(avs_coap_ctx_t *ctx)
{
    ...
    if (!coap_base->request_ctx.response_setup && is_entire_request_finished(&coap_base->request_ctx.request))
    {
        LOG(DEBUG,
                               ...
                               int exchange_request_handler_result)
{
    ...
    if (is_request_message_finished(&coap_base->request_ctx.request) || coap_base->request_ctx.response_setup)
    {
                                  ...
                                  AVS_LIST(avs_coap_exchange_t) * out_exchange)
{
    ...
    AVS_ASSERT(request->payload_offset + request->payload_size <= request->total_payload_size,
    ...
    assert(server_data);
    assert(server_data->request_key_options.capacity >= _avs_coap_options_request_key_size(&request->options));
    ...
    AVS_LIST(struct avs_coap_exchange) * out_exchange)
{
    ...
    avs_error_t err =
        ctx->vtable->receive_message(ctx, in_buffer, in_buffer_size,
                                     &coap_base->request_ctx.request);
    if (avs_is_ok(err) && avs_coap_code_is_request(coap_base->request_ctx.request.code))
    {
        assert(coap_base->request_ctx.request.payload_size > 0 || coap_base->request_ctx.request.payload_offset == coap_base->request_ctx.request.total_payload_size);
...
avs_error_t avs_coap_async_handle_incoming_packet(
    ...
    void *on_new_request_arg)
{
    printf("In avs_coap_async_handle_incoming_packet");
...
void _avs_coap_server_exchange_cleanup(avs_coap_ctx_t *ctx,
                                       ...
                                       avs_error_t err)
{
    printf("_avs_coap_server_exchange_cleanup\n");
    avs_coap_base_t *coap_base = _avs_coap_get_base(ctx);
                      ...
                      void *delivery_handler_arg)
{
    ...
    if (!delivery_handler && reliability_hint != AVS_COAP_NOTIFY_PREFER_NON_CONFIRMABLE)
    {
        LOG(ERROR,
    ...
    server_exchange_create_args_t exchange_create_args = {
        ...
        .observe_option_value = avs_coap_code_is_success(response_header->code) ? &notify.observe_option_value : NULL};
    AVS_LIST(avs_coap_exchange_t)
    exchange =
        server_exchange_create(&exchange_create_args);
    if (!exchange)

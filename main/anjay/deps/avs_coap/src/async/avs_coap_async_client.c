...
#ifdef WITH_AVS_COAP_BLOCK
    if ((**exchange_ptr_ptr)->by_type.client.next_response_payload_offset > 0 && !_avs_coap_options_find_first_opt(&(**exchange_ptr_ptr)->options,
                                                                                                                   AVS_COAP_OPTION_BLOCK2))
    {
        const size_t block_size =
            initial_block2_option_size(ctx, **exchange_ptr_ptr);
        if ((**exchange_ptr_ptr)->by_type.client.next_response_payload_offset >= block_size)
        {
                               ...
                               state_with_error_t request_state)
{
    ...
    size_t expected_payload_offset =
        AVS_MIN(exchange->by_type.client.next_response_payload_offset,
                response_payload_offset + (response_msg ? response_msg->payload_size : 0));
    assert(expected_payload_offset >= response_payload_offset);
    assert(expected_payload_offset - response_payload_offset <= (response_msg ? response_msg->payload_size : 0));
    const avs_coap_client_async_response_t *exchange_response =
        !response_msg ? NULL
                      : &(const avs_coap_client_async_response_t){
                            ...
                            .payload = (const char *)response_msg->payload + (expected_payload_offset - response_payload_offset),
                            .payload_size = response_msg->payload_size - (expected_payload_offset - response_payload_offset)};
                              ...
                              const avs_coap_borrowed_msg_t *response)
{
    ...
    if (response->payload)
    {
        while (*exchange_ptr_ptr && response_payload_offset + response->payload_size > (**exchange_ptr_ptr)
                                                                                           ->by_type.client
                                                                                           .next_response_payload_offset)
        {
            size_t expected_payload_offset =
            ...
            if ((*exchange_ptr_ptr = _avs_coap_find_client_exchange_ptr_by_id(
                ...
                (**exchange_ptr_ptr)
                        ->by_type.client
                        .next_response_payload_offset == expected_payload_offset)
            {
                (**exchange_ptr_ptr)
                    ->by_type.client.next_response_payload_offset =
...
exchange_expects_continue_response(const avs_coap_exchange_t *exchange)
{
    ...
    return avs_coap_code_is_request(exchange->code) && avs_coap_options_get_block(&exchange->options, AVS_COAP_BLOCK1, &request_block1) == 0 && request_block1.has_more;
}
    ...
    const avs_coap_option_block_t *response_block1)
{
    ...
    avs_error_t err;
    if (response_block1 && avs_is_err((err = handle_request_block_size_renegotiation(
                         ...
                         const avs_coap_borrowed_msg_t *response)
{
    ...
    case 0:
    {
        ...
        assert(!*exchange_ptr_ptr || **exchange_ptr_ptr);
        if (*exchange_ptr_ptr && avs_is_err((err = client_exchange_send_next_chunk(
                                                 ctx, exchange_ptr_ptr))))
        {
            return failure_state(err);
        }
    ...
    const avs_coap_option_block_t *response_block2)
{
    ...
    if (avs_is_err((err = avs_coap_options_add_block(&exchange->options,
                                                     &block2))))
    {
        AVS_ASSERT(err.category != AVS_COAP_ERR_CATEGORY || err.code != AVS_COAP_ERR_MESSAGE_TOO_BIG,
                      ...
                      const avs_coap_borrowed_msg_t *response)
{
    ...
    case 0:
    {
        ...
        size_t request_off = request_has_block2 ? request_block2.seq_num * request_block2.size
                                                : 0;
        ...
        if (response_block2.has_more)
        {
            ...
            if (*exchange_ptr_ptr)
            {
                ...
                if (err.category == AVS_ERRNO_CATEGORY && err.code == AVS_ERANGE)
                {
                ...
                if (avs_is_err(err) || avs_is_err((err = client_exchange_send_next_chunk(
                                                       ctx, exchange_ptr_ptr))))
                {
                    return failure_state(err);
                }
                         ...
                         AVS_LIST(avs_coap_exchange_t) * *exchange_ptr_ptr)
{
    ...
    while (avs_is_ok(err) && *exchange_ptr_ptr && !(**exchange_ptr_ptr)->eof_cache.empty)
    {
                      ...
                      avs_coap_exchange_id_t *out_id)
{
    ...
    while (*insert_ptr && !_avs_coap_client_exchange_request_sent(*insert_ptr))
    {
...
{
    assert(exchange_ptr_ptr && *exchange_ptr_ptr && **exchange_ptr_ptr);
    if (fail_err.category != AVS_COAP_ERR_CATEGORY || fail_err.code != AVS_COAP_ERR_TRUNCATED_MESSAGE_RECEIVED || !response)
    {
    ...
    void *response_handler_arg)
{
    ...
    *exchange = (avs_coap_exchange_t){
        ...
        .eof_cache = {
            .empty = true},
        .by_type = {.client = {.handle_response = response_handler, .handle_response_arg = response_handler_arg, .send_result_handler = response_handler ? on_request_delivery_finished : NULL, .send_result_handler_arg = exchange, .next_response_payload_offset = next_response_payload_offset}},
    ...
    size_t next_response_payload_offset)
{
    ...
    if (next_response_payload_offset <= exchange->by_type.client.next_response_payload_offset && (next_response_payload_offset > 0 || exchange->token.size > 0))
    {
        return avs_errno(AVS_EINVAL);
    }

...
{
    return ctx->state == AVS_COAP_STREAMING_SERVER_RECEIVED_REQUEST_CHUNK || ctx->state == AVS_COAP_STREAMING_SERVER_RECEIVED_LAST_REQUEST_CHUNK;
}

static bool
is_sending_response_chunk(const avs_coap_streaming_server_ctx_t *ctx)
{
    return ctx->state == AVS_COAP_STREAMING_SERVER_SENDING_FIRST_RESPONSE_CHUNK || ctx->state == AVS_COAP_STREAMING_SERVER_SENDING_RESPONSE_CHUNK;
}
                              ...
                              void *streaming_server_ctx_)
{
    ...
    AVS_ASSERT(streaming_server_ctx->expected_next_outgoing_chunk_offset == payload_offset,
                           ...
                           void *streaming_req_ctx_)
{
    ...
    assert(request->payload_size <= avs_buffer_space_left(
    ...
    assert(request_ctx == &_avs_coap_get_base(streaming_req_ctx->server_ctx.coap_ctx)
                               ->request_ctx);
    assert(streaming_req_ctx->server_ctx.state == AVS_COAP_STREAMING_SERVER_RECEIVING_REQUEST);
                              ...
                              void *streaming_req_ctx_)
{
    ...
    assert(avs_coap_exchange_id_valid(streaming_req_ctx->server_ctx.exchange_id) == !!streaming_req_ctx->server_ctx.chunk_buffer);
    if (!streaming_req_ctx->server_ctx.chunk_buffer)
    {
...
coap_write(avs_stream_t *stream_, const void *data, size_t *data_length)
{
    ...
    if (avs_is_ok(err) && !is_sending_response_chunk(&streaming_req_ctx->server_ctx))
    {
    ...
    while (bytes_written < *data_length)
    {
        ...
        if (avs_buffer_space_left(streaming_req_ctx->server_ctx.chunk_buffer) == 0 && avs_is_err((streaming_req_ctx->err = flush_response_chunk(
                                                                                                      streaming_req_ctx))))
        {
                       ...
                       avs_time_duration_t recv_timeout)
{
    ...
    if (exchange)
    {
        ...
        if (!call_result && is_streaming_request && has_received_request_chunk(&streaming_req_ctx->server_ctx))
        {
    ...
    avs_coap_streaming_request_ctx_t *streaming_req_ctx)
{
    ...
    if (streaming_req_ctx->server_ctx.state == AVS_COAP_STREAMING_SERVER_RECEIVED_REQUEST_CHUNK && avs_buffer_data_size(streaming_req_ctx->server_ctx.chunk_buffer) == 0)
    {
        ...
        while (streaming_req_ctx->server_ctx.state == AVS_COAP_STREAMING_SERVER_RECEIVING_REQUEST)
        {
            avs_time_monotonic_t next_deadline =
                _avs_coap_retry_or_request_expired_job(
                    streaming_req_ctx->server_ctx.coap_ctx);
            if (streaming_req_ctx->server_ctx.state != AVS_COAP_STREAMING_SERVER_RECEIVING_REQUEST)
            {
                // The exchange has been cleaned up by
                // _avs_coap_retry_or_request_expired_job()
                assert(streaming_req_ctx->server_ctx.state == AVS_COAP_STREAMING_SERVER_FINISHED);
    ...
    if (out_message_finished)
    {
        *out_message_finished =
            (avs_buffer_data_size(
                 streaming_req_ctx->server_ctx.chunk_buffer) == 0 &&
             streaming_req_ctx->server_ctx.state == AVS_COAP_STREAMING_SERVER_RECEIVED_LAST_REQUEST_CHUNK);
    }
    return AVS_OK;
}
    ...
    while (true)
    {
                           ...
                           &streaming_req_ctx, AVS_TIME_DURATION_ZERO))))
        {
            ...
            if (has_received_request_chunk(&streaming_req_ctx.server_ctx))
            {
                ...
                if (!streaming_req_ctx.error_response_code && has_received_request_chunk(
            ...
            while (streaming_req_ctx.server_ctx.state != AVS_COAP_STREAMING_SERVER_FINISHED)
            {
...
notify_write(avs_stream_t *stream_, const void *data, size_t *data_length)
{
    ...
    while (bytes_written < *data_length)
    {
        ...
        if (avs_buffer_space_left(notify_streaming_ctx->server_ctx.chunk_buffer) == 0)
        {
                          ...
                          void *write_payload_arg)
{
    ...
    while (avs_is_ok(notify_streaming_ctx.err) && notify_streaming_ctx.server_ctx.state != AVS_COAP_STREAMING_SERVER_FINISHED)
    {
        notify_streaming_ctx.err = flush_notify_chunk(&notify_streaming_ctx);
    }

    if (avs_is_err(notify_streaming_ctx.err) && avs_coap_exchange_id_valid(
                                                    notify_streaming_ctx.server_ctx.exchange_id))
    {
        ...
        if (notify_streaming_ctx.server_ctx.state != AVS_COAP_STREAMING_SERVER_FINISHED)
        {
            avs_coap_exchange_cancel(
                ctx, notify_streaming_ctx.server_ctx.exchange_id);
        }
    }
    assert(notify_streaming_ctx.server_ctx.state == AVS_COAP_STREAMING_SERVER_FINISHED);

finish:
    avs_buffer_free(&notify_streaming_ctx.server_ctx.chunk_buffer);
    if (avs_is_ok(notify_streaming_ctx.err) && notify_streaming_ctx.required_receiving)
    {
        notify_streaming_ctx.err =

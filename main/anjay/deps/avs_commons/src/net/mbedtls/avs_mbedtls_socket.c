...
#if defined(AVS_COMMONS_WITH_AVS_CRYPTO_PKI) && defined(MBEDTLS_SHA256_C) && defined(MBEDTLS_SHA512_C)
...
#define DANE_TA_OR_EE_MATCH_MASK \
    ((1 << AVS_NET_SOCKET_DANE_TRUST_ANCHOR_ASSERTION) | (1 << AVS_NET_SOCKET_DANE_DOMAIN_ISSUED_CERTIFICATE))

#define DANE_FULL_MATCH_MASK \
    (DANE_TA_OR_EE_MATCH_MASK | (1 << AVS_NET_SOCKET_DANE_CA_CONSTRAINT) | (1 << AVS_NET_SOCKET_DANE_SERVICE_CERTIFICATE_CONSTRAINT))
...
avs_bio_recv(void *ctx, unsigned char *buf, size_t len, uint32_t timeout_ms)
{
    ...
    {
        if (socket->bio_error.category == AVS_ERRNO_CATEGORY && socket->bio_error.code == AVS_ETIMEDOUT)
        {
                                     ...
                                     int *out_padding_size)
{
    ...
    if (result == MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE || result == MBEDTLS_ERR_SSL_INTERNAL_ERROR)
    {
...
#if defined(AVS_COMMONS_WITH_AVS_CRYPTO_PKI) || defined(AVS_COMMONS_NET_WITH_PSK)
static bool
                              ...
                              const avs_net_socket_dane_tlsa_record_t *entry)
{
    ...
    case AVS_NET_SOCKET_DANE_MATCH_FULL:
        return entry->association_data_size == buf_len && memcmp(entry->association_data, buf, buf_len) == 0;
    case AVS_NET_SOCKET_DANE_MATCH_SHA256:
    {
        ...
        return !mbedtls_sha256(buf, buf_len, sha, 0) && memcmp(entry->association_data, sha, sizeof(sha)) == 0;
    }
    case AVS_NET_SOCKET_DANE_MATCH_SHA512:
    {
        ...
        return !mbedtls_sha512(buf, buf_len, sha, 0) && memcmp(entry->association_data, sha, sizeof(sha)) == 0;
    }
    }
                       ...
                       const avs_net_socket_dane_tlsa_record_t *entry)
{
    ...
    case AVS_NET_SOCKET_DANE_PUBLIC_KEY:
    {
        ...
        bool result =
            (length >= 0 && dane_match_buffer(&buf[raw_crt_size - (size_t)length],
                                              (size_t)length, entry));
        avs_free(buf);
        return result;
    }
...
static bool has_dane_ta_or_ee_entries(ssl_socket_t *socket)
{
         ...
         ++i)
    {
        ...
        if (entry->certificate_usage == AVS_NET_SOCKET_DANE_TRUST_ANCHOR_ASSERTION || entry->certificate_usage == AVS_NET_SOCKET_DANE_DOMAIN_ISSUED_CERTIFICATE)
        {
            return true;
        }
    }
                                     ...
                                     uint32_t verify_result_flags)
{
         ...
         ++i)
    {
        const avs_net_socket_dane_tlsa_record_t *const entry =
            &socket->cert_security.dane_tlsa.array_ptr[i];
        if (socket->cert_security.dane_verify_state.match_mask & (1 << entry->certificate_usage))
        {
            // Certificate usage already satisfied, no need to check again
            continue;
        }
        if ((entry->certificate_usage == AVS_NET_SOCKET_DANE_SERVICE_CERTIFICATE_CONSTRAINT || entry->certificate_usage == AVS_NET_SOCKET_DANE_DOMAIN_ISSUED_CERTIFICATE) != is_ee)
        {
            // TA/EE mismatch
            continue;
        }
...
static uint32_t perform_cert_verification(ssl_socket_t *socket)
{
    ...
    if (socket->cert_security.dane_verify_state.verify_result_flags == MBEDTLS_X509_BADCERT_NOT_TRUSTED && (socket->cert_security.dane_verify_state.match_mask & DANE_TA_OR_EE_MATCH_MASK))
    {
        return 0;
    }

    uint8_t dane_valid_matches = (socket->cert_security.ca_cert == socket->cert_security.dane_ta_certs)
                                     ? DANE_TA_OR_EE_MATCH_MASK
                                     : DANE_FULL_MATCH_MASK;
    // If verification succeeded,
    // check if DANE verification succeeded as well
    if (!socket->cert_security.dane_verify_state.verify_result_flags && socket->cert_security.dane_tlsa.array_element_count > 0 && !(socket->cert_security.dane_verify_state.match_mask & dane_valid_matches))
    {
                          ...
                          uint32_t *verify_result_flags)
{
    ...
    if (socket->cert_security.ca_cert == socket->cert_security.dane_ta_certs && !has_dane_ta_or_ee_entries(socket))
    {
...
{
    if (result >= 0 && socket->security_mode == AVS_NET_SECURITY_CERTIFICATE && socket->cert_security.dane_ta_certs && socket->cert_security.dane_verify_state.verify_result)
    {
...
static avs_error_t update_cert_configuration(ssl_socket_t *socket)
{
    ...
    if (socket->cert_security.dane_ta_certs)
    {
             ...
             ++i)
        {
            const avs_net_socket_dane_tlsa_record_t *const entry =
                &socket->cert_security.dane_tlsa.array_ptr[i];
            if (entry->certificate_usage == AVS_NET_SOCKET_DANE_TRUST_ANCHOR_ASSERTION && entry->selector == AVS_NET_SOCKET_DANE_CERTIFICATE && entry->matching_type == AVS_NET_SOCKET_DANE_MATCH_FULL && mbedtls_x509_crt_parse_der(socket->cert_security.dane_ta_certs, (const unsigned char *)entry->association_data, entry->association_data_size))
            {
    ...
    const avs_net_socket_tls_ciphersuites_t *enabled_ciphers)
{
    ...
    for (const int *cipher = all_ciphers; cipher && *cipher; ++cipher)
    {
        const mbedtls_ssl_ciphersuite_t *info =
            mbedtls_ssl_ciphersuite_from_id(*cipher);
        if (mbedtls_ssl_ciphersuite_uses_psk(info) && contains_cipher(enabled_ciphers, *cipher))
        {
            ++ciphers_count;
        }
    }
                           ...
                           const mbedtls_ssl_session *right)
{
    ...
    return left && right && left->ciphersuite == right->ciphersuite && left->compression == right->compression
#ifdef MBEDTLS_HAVE_TIME
           && left->start == right->start
#endif // MBEDTLS_HAVE_TIME
           && left->id_len == right->id_len && memcmp(left->id, right->id, left->id_len) == 0;
}
              ...
              const avs_net_ssl_configuration_t *configuration)
{
    ...
    if (avs_time_duration_to_scalar(&min_ms, AVS_TIME_MS,
        ...
        min_ms < 0 || min_ms > UINT32_MAX || max_ms < 0 || max_ms > UINT32_MAX)
    {
    ...
    if (socket->use_connection_id && transport_for_socket_type(socket->backend_type) == MBEDTLS_SSL_TRANSPORT_DATAGRAM && mbedtls_ssl_conf_cid(&socket->config, 0, MBEDTLS_SSL_UNEXPECTED_CID_IGNORE))
    {
    ...
    if (configuration->additional_configuration_clb && configuration->additional_configuration_clb(&socket->config))
    {
...
static avs_error_t start_ssl(ssl_socket_t *socket, const char *host)
{
    ...
    if (socket->use_connection_id && transport_for_socket_type(socket->backend_type) == MBEDTLS_SSL_TRANSPORT_DATAGRAM && mbedtls_ssl_set_cid(get_context(socket), MBEDTLS_SSL_CID_ENABLED, NULL, 0))
    {
...
#ifdef AVS_COMMONS_NET_WITH_TLS_SESSION_PERSISTENCE
    if (socket->session_resumption_buffer && endpoint == MBEDTLS_SSL_IS_CLIENT)
    {
    ...
    do
    {
        printf("About to perform mbedtls handshale\n");
    ...
    if (result == 0)
    {
        ...
        if (!socket->flags.session_fresh && !sessions_equal(get_context(socket)->session,
                                                            &restored_session))
        {
            socket->flags.session_fresh = true;
        }
#endif // MBEDTLS_SSL_SRV_C
        if (socket->session_resumption_buffer && socket->flags.session_fresh && endpoint == MBEDTLS_SSL_IS_CLIENT)
        {
        ...
        {
            printf("handshake success: new session started\n");
            LOG(TRACE, _("handshake success: new session started"));
        }
        else
        {
            printf("handshake success: session restored\n");
...
send_ssl(avs_net_socket_t *socket_, const void *buffer, size_t buffer_length)
{
    ...
    while (bytes_sent < buffer_length)
    {
        ...
        } while (result == MBEDTLS_ERR_SSL_WANT_WRITE || result == MBEDTLS_ERR_SSL_WANT_READ);
        result = wrap_handshake_result(socket, result);
        if (result <= 0)
        {
            break;
                               ...
                               size_t buffer_length)
{
    ...
    if (buffer_length > 0 && transport_for_socket_type(socket->backend_type) == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
    ...
    else
    {
        *out_bytes_received = (size_t)result;
        if (transport_for_socket_type(socket->backend_type) == MBEDTLS_SSL_TRANSPORT_DATAGRAM && mbedtls_ssl_get_bytes_avail(get_context(socket)) > 0)
        {
                                               ...
                                               uint32_t *verify_result_flags)
{
    ...
    if (index == 0)
    {
        ...
        assert(crt == last_configured_cert || (raw_crt_size == last_raw_crt_size && memcmp(raw_crt, last_raw_crt, raw_crt_size) == 0));
#endif // NDEBUG
        return 0;
    }
                                      ...
                                      const mbedtls_x509_crt *checked_cert)
{
    ...
    while (cert && _avs_crypto_mbedtls_x509_crt_present(cert))
    {
        ...
        if (raw_cert_size == raw_checked_cert_size && memcmp(raw_cert, raw_checked_cert, raw_cert_size) == 0)
        {
            return true;
        }
                                             ...
                                             mbedtls_x509_crt *first_cert)
{
    ...
    while (_avs_crypto_mbedtls_x509_crt_present(last_cert) && *_avs_crypto_mbedtls_x509_crt_next_ptr(last_cert))
    {
    ...
    while (true)
    {
        ...
        else if (!_avs_crypto_mbedtls_x509_crt_present(last_cert) || !*_avs_crypto_mbedtls_x509_crt_next_ptr(last_cert))
        {
                    ...
                    avs_crypto_prng_ctx_t *prng_ctx)
{
    ...
    if ((cert_info->server_cert_validation || cert_info->rebuild_client_cert_chain) && avs_is_err((err = _avs_crypto_mbedtls_load_certs(
    ...
    if (avs_is_ok(err))
    {
        if (cert_info->client_cert.desc.source != AVS_CRYPTO_DATA_SOURCE_EMPTY)
        {
            ...
            else if (cert_info->rebuild_client_cert_chain && ca_certs && certs->client_cert && _avs_crypto_mbedtls_x509_crt_present(certs->client_cert) && avs_is_err((err = rebuild_client_cert_chain(ca_certs, certs->client_cert))))
            {
                LOG(ERROR, _("could not rebuild client certificate chain"));
            }
            if (avs_is_ok(err) && avs_is_err(
